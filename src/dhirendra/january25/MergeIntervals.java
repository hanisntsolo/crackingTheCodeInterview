...
//}
    //Working code
public int[][] merge(int[][] intervals) {
    //new array to store non overlapping intervals
    int [][] newIntervals = new int[intervals.length-1][2];
    ...
    for(int i=1;i<intervals.length;i++) {
        if(intervals[i-1][1]>=intervals[i][0]) {
            newIntervals[i-1][0] = intervals[i-1][0];
            newIntervals[i-1][1] = intervals[i][1];
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(Arrays.stream(intervals).toArray()));
    }
}
